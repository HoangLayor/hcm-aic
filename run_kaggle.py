...
def remove_batch_segment_videos(video_ids: list):
    """Remove VAD-generated MP4 files after a batch has been ingested to Qdrant."""
    output_root = Path(config.OUTPUT_DIR)
    removed_count = 0
    for video_id in video_ids:
        segments_dir = output_root / video_id / "segments"
        if not segments_dir.is_dir():
            logger.warning(f"Segments directory not found; nothing to clean: {segments_dir}")
            continue
        for segment_file in segments_dir.glob("*.mp4"):
            segment_file.unlink()
            removed_count += 1
    logger.info(f"Removed {removed_count} VAD segment MP4 file(s) after Qdrant ingestion.")
